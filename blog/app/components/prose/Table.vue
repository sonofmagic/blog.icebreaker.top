...
function requestScrollabilityUpdate() {
  if (!import.meta.client) {
    return
  }
  window.requestAnimationFrame(updateScrollability)
}

onMounted(async () => {
  await nextTick()
  updateScrollability()
  window.addEventListener('resize', requestScrollabilityUpdate, { passive: true })

  if ('ResizeObserver' in window && tableScroll.value) {
    resizeObserver = new ResizeObserver(requestScrollabilityUpdate)
    resizeObserver.observe(tableScroll.value)
  }
})

onBeforeUnmount(() => {
  window.removeEventListener('resize', requestScrollabilityUpdate)
...
.prose-table-scroll {
  ...
  background:
    ...
    linear-gradient(270deg, var(--panel-bg) 30%, color-mix(in srgb, var(--panel-bg) 0%, transparent)) right center /
      1rem 100% no-repeat,
    linear-gradient(90deg, rgba(15, 23, 42, 0.09), transparent) left center / 0.65rem 100% no-repeat,
    linear-gradient(270deg, rgba(15, 23, 42, 0.09), transparent) right center / 0.65rem 100% no-repeat;
